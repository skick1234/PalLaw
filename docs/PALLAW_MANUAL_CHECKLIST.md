...
- Verify the homepage and `favicon.svg` are crawlable, and the favicon returns HTTP 200 with `image/svg+xml`. Keep its URL stable. The square SVG supports arbitrary display sizes; an Apple touch icon requires a separate PNG. See [Google's favicon guidance](https://developers.google.com/search/docs/appearance/favicon-in-search).
